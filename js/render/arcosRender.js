...
export function renderArcos (arcos, container, startColor = 0){

//const arcosGrid = document.querySelector(".arcos-grid"); //cria uma variável para armazenar o que for selecionado na classe grid

arcos.forEach((arcoInfo, index) => { //para cada card do array ele vai fazer o seguinte:
  const realColor = startColor + index;
  ...
  const cardArcosCor = cardCores [realColor % cardCores.length];
...
if (arcoInfo.crown === true) {
  ...
  container.appendChild(arcosCard); //isso é o que fará o card aparecer na página
});
}